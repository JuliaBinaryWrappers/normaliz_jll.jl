...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import normaliz_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("6690c6e9-4e12-53b8-b8fd-4bffaef8839f"))
platforms = [Pkg.Artifacts.unpack_platform(e, "normaliz", artifacts_toml) for e in artifacts["normaliz"]]
...
if best_platform === nothing
    @debug("Unable to load normaliz; unsupported platform $(triplet(platform_key_abi()))")
else
